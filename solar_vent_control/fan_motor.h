...
class fan_motor {
  ...
  public:
    ...
    static void speeds_fans_2s(int speed, fan_motor& fan_1st, fan_motor& fan_2st, bool preoritet); 
                                                     ...
                                                     // ещё надо передать объекты первого и второго мотора
                                                  // (не ссылки, не указатели)
                                                     // если приоритет = 0, то первым включится  вентиятор 1, и скорость прибавится первым у него, если 1, то наоборот у второго
